...
//if suduko is not completed or incorrect then moving submit btn functionality
showerrorfun()
if (error > 0 && emptyplaces > 0) {
    var b = 0
    var mousepoint = document.getElementById('submit')
    mousepoint.addEventListener('mouseover', function () {
        if (b == 0) {
            b = 1;
            mousepoint.style.marginLeft = "57%"
            // mousepoint.style.backgroundColor = "red"
        }
        else {
            b = 0;
            mousepoint.style.marginLeft = "37%"
            // mousepoint.style.backgroundColor = "green"
        }
    })
    const sbtn = document.getElementById('submitbtn').addEventListener('mouseleave', function () {
        mousepoint.style.marginLeft = "47%"
    })
}
submitfun = () => {
    showerrorfun()
